...
> You should now see `.onboarding/example-report.md`. This shows how OpenWork can pull read-only tool data or a template into a saved workspace report. Any questions before I continue with the workstation readiness check?

Wait for the user to continue before running the workstation readiness check.
...
### 11. Invoke personal memory setup

Now that the system prerequisites and workstation tools are in place, use the `personal-memory` skill to help the teammate initialize and populate safe personal OpenWork memory. Do this before writing the final onboarding report so the report can record the memory setup outcome.
...
### 12. Write final onboarding report
...
- Mention whether personal memory was set up, declined, or skipped, and the memory paths created.
...
- [ ] Asked before initializing or populating personal memory, immediately after the workstation readiness/prerequisite step.
...
- [ ] Wrote `.onboarding/openwork-onboarding-report.md` after memory setup.
