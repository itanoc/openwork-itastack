...
metadata:
  route_default: daily
  route_max: daily
  route_class: report_issue
---

<<<ROUTE default=daily max=daily class=report_issue>>>
...
Read reporter identity from `memory/preferences/current-openwork-teammate.md` when available.

- Look for a simple profile line like `- Name: Jane Teammate` or `Name: Jane Teammate`.
