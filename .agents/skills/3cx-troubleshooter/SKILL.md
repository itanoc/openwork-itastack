...
metadata:
  ...
  route_max: high
  route_class: 3cx_troubleshooter
---

<<<ROUTE default=medium max=high class=3cx_troubleshooter>>>
...
- Offline 3CX documentation mirror (`documentation/index.md`) — always read this index first and name the matching doc(s) before web research. Keep this lookup quick; it does not replace producing the full deliverable below.
...
### Offline 3CX documentation mirror

- A local mirror of 75 3CX documentation pages lives under `documentation/` alongside this skill file.
- Always consult `documentation/index.md` first to discover relevant docs before searching the web.
- The index is organized by section: AI Features, Advanced, General, Data Connector Guides, Hotel Integration, IP Phone Related Guides, Security, Troubleshooting.
- Map the issue domain to the matching section(s), then open the specific doc(s) that cover the symptom.
- Every mirrored doc includes a `> Source:` line with the original 3CX URL — use that for live verification when the local copy may be stale.
- If no relevant doc exists in the mirror, fall back to web research.
...
### 4. Cross-reference offline documentation and knowledge base

- First, read `documentation/index.md` to identify which section(s) match the symptom domain.
- Open the relevant mirrored doc(s) from the `documentation/` directory for authoritative configuration, troubleshooting, and reference guidance.
- Map common issue domains to sections:
  - AI/transcription: `ai-features/`
  - Failover, SBC HA, SSL, SSO, M365/Google integration, CDR, TAPI, APIs, provisioning: `advanced/`
  - Bridges, upgrades, caller ID, hot desking, dial codes, inbound/outbound rules, events, reports: `general/`
  - Grafana, BigQuery, PostgreSQL, MSSQL, MySQL offload: `data-connector/`
  - PMS, hotel module, billing, wake-up calls: `hotel-integration/`
  - Deskphones, BLF, firmware, provisioning templates, SIP-TLS, factory reset: `ip-phone/`
  - Credential reset, IP allow/deny: `security/`
  - Log collection, log viewer, Android push, Teams integration, packet capture, DID troubleshooting, VoIP provider checker: `troubleshooting/`
- Use other workspace KB/reference documents as secondary offline reference when available.
- If the mirrored doc conflicts with current 3CX release notes or live evidence, note the version on the live page vs the mirrored copy and prefer the live page.
- If no relevant doc is found in the mirror or KB, say so and fall back to web research.
- Do not invent doc content. If no source was found, say so.
...
Output the full deliverable INLINE in your reply, with every section heading shown. Never save the deliverable to a file, never write it to disk, and never hand it to another agent in place of replying. A file path is not an answer — the technician must see all sections directly in your response.

Always finish your reply with all the section headings below, even if investigation is incomplete. Do not stop after tool calls or analysis. If a step is unfinished or evidence is missing, still write every section using what you know and label the gaps under Evidence Found. Producing all sections is the last thing you do before ending your turn.
