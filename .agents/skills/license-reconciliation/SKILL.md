...
metadata:
  route_default: daily
  route_max: medium
  route_class: license_reconciliation
---

<<<ROUTE default=daily max=medium class=license_reconciliation>>>
...
If a tenant has no `SPB` row, do not count legacy `O365_BUSINESS_PREMIUM` as a target match. Put that tenant in `Next Actions` with the exact missing fact, such as `No M365 SPB target SKU found; legacy O365_BUSINESS_PREMIUM ignored by rule`.
...
For `BUSINESS_PREMIUM`, call `itastack_itastack_grafana` operation `query_halo_sql` with `params.sql` and `params.limit`. Use `sql`, not `query`, or the dispatcher will fail.

Call shape:

```json
{"operation":"query_halo_sql","params":{"sql":"<SQL below>","limit":300}}
```
...
| `cpti` | Columbia Power |
| `lcc` | Life Care |
| `omep` | Oregon Manufacturing, OMEP |
...
- When multiple active target-shaped rows exist but stale status is not proven, keep category `Duplicate/Stale Halo Row` but phrase rationale as `duplicate active target-shaped rows; stale status unverified`.
...
Omit this section entirely when there is no prior report reclassification.
